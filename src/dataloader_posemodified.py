...
# def get_pose(data, path, seqlen, framerate):
#     pose_pack = []
                ...
                for file in os.listdir(img_folder_path):
                    ...
                    if os.path.isfile(file_dir):
                        if os.path.isfile(file_end_dir):
                            pair = (file_dir, (num, folder[:-4]), seg_dir)
                            images.append(pair)
                    else:
                        raise LookupError("Direction %s doesnt exist."%file_dir)
...
class human36(data.Dataset):
    def __init__(self, opt, train, transform=None):
        ...
        if train==True:
            ...
            # self.pose_data = pose_loader(opt.dataroot, opt.train_subjectlist, opt.actionlist)
        else:
            img_list = make_imglist(opt.dataroot, opt.test_subjectlist, opt.actionlist, opt.seqlen, opt.framerate)
            # self.pose_data = pose_loader(opt.dataroot, opt.test_subjectlist, opt.actionlist)
        self.imgs = img_list
