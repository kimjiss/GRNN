...
def GRNN_trainer(opt, train_dataloader, test_dataloader):
    ...
    for epoch in range(opt.niter):
        ...
        for i, (image, seg) in enumerate(train_dataloader):
            ...
            #######################
            # Real
            input = [torch.cat([pre_img, post_img], dim=1) for pre_img, post_img in zip(image[:-1], image[1:])]
            input = Variable(torch.cat(input, dim=0)).cuda()
            label = Variable(torch.FloatTensor(input.size(0)).fill_(real)).cuda()
            out = Dnet(input)
            errD_real = Dcriterion(out, label)
            ...
            input = [torch.cat([pre_img, post_img], dim=1) for pre_img, post_img in zip(input_g[:-1], out)]
            input = torch.cat(input, dim=0)
            out_ = Dnet(input)
            label = Variable(torch.FloatTensor(input.size(0)).fill_(fake)).cuda()
            errD_fake = Dcriterion(out_, label)
            ...
            net.zero_grad()
            out_ = Dnet(input)
            segment = [(Variable(item.cuda()) + 0.2).expand_as(out[0]) for item in seg]
            D_g_2 = out_.data.mean()
            label = Variable(torch.FloatTensor(input.size(0)).fill_(real)).cuda()
            errG = criterion(out_, label)
            err = [torch.mean(torch.abs(y * seg - y_)) for y, y_, seg in zip(out, input_g[1:], segment[1:])]
            err = torch.cat(err).mean() + errG
            err.backward()
            optimizer.step()
            ...
            if i % 20 == 0:
                print('[%d/%d][%d/%d] Loss_G: %.4f , Loss_D: %.4f , D(X): %.4f , D(G(x)): %.4f / %.4f'
                      % (epoch, opt.niter, i, len(train_dataloader),
                         err.data[0], errD.data[0], D_x, D_g_1, D_g_2))
            if i % 125 == 0:
