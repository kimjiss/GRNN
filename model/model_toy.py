...
def GRNN_trainer(opt, train_dataloader, test_dataloader):
    ...
    optimizerD = optim.Adam(Dnet.parameters(), lr=opt.lr*0.7, betas=(opt.beta1, 0.999))
    ...
    for epoch in range(opt.niter):
        ...
        for i, image in enumerate(train_dataloader):
            ...
            input_ = torch.cat(input, dim=2)
            out_ = Dnet(input_).squeeze()
            label = Variable(torch.FloatTensor(input_real.size(0)).fill_(fake)).cuda()
